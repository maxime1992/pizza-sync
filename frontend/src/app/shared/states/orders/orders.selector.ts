...
import { IOrdersSummary, IPizzaOrderSummary, IOrdersTable } from 'app/shared/states/orders/orders.interface';
...
export function _getOrderSummary(ordersTable: IOrdersTable, pizzasTable: IPizzasTable): IOrdersSummary {
  const ordersSummaryMap = ordersTable
    .allIds
    .reduce((acc: { [key: string]: IPizzaOrderSummary }, orderId: string) => {
...
export function getOrderSummary(store$: Store<IStore>): Observable<IOrdersSummary> {
  return store$
    .select(state => ({ orders: state.orders, pizzas: state.pizzas }))
    .distinctUntilChanged((p, n) => p.pizzas === n.pizzas && p.orders === n.orders)
